        ...
        for key,vals in data_group_month.groups.items():
            ...
            for i in group_by_data_user.index:
                PRODUCT_SHEET_RANGES = [
                    {'product_uom_qty': group_by_data_user[4][i], 'default_code': group_by_data_user[7][i], 'name': group_by_data_user[8][i]},
                    {'product_uom_qty': group_by_data_user[9][i], 'default_code': group_by_data_user[10][i], 'name': group_by_data_user[11][i]},
                    {'product_uom_qty': group_by_data_user[12][i], 'default_code': group_by_data_user[13][i], 'name': group_by_data_user[14][i]},
                    {'product_uom_qty': group_by_data_user[15][i], 'default_code': group_by_data_user[16][i], 'name': group_by_data_user[17][i]},
                    {'product_uom_qty': group_by_data_user[18][i], 'default_code': group_by_data_user[19][i], 'name': group_by_data_user[20][i]},
                    {'product_uom_qty': group_by_data_user[21][i], 'default_code': group_by_data_user[22][i], 'name': group_by_data_user[23][i]},
                    {'product_uom_qty': group_by_data_user[24][i], 'default_code': group_by_data_user[25][i], 'name': group_by_data_user[26][i]},
                    {'product_uom_qty': group_by_data_user[27][i], 'default_code': group_by_data_user[28][i], 'name': group_by_data_user[29][i]},
                    {'product_uom_qty': group_by_data_user[30][i], 'default_code': group_by_data_user[31][i], 'name': group_by_data_user[32][i]},
                ]
                for product_data in PRODUCT_SHEET_RANGES:
                    ...
                    if product_id:
                        product_vals = {
                            ...
                            'product_uom_qty': product_data['product_uom_qty'] if product_data['product_uom_qty'] else 0,
